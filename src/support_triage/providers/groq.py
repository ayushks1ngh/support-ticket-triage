"""Groq adapter through Strands' supported OpenAI-compatible provider."""
...
class GroqProvider(LLMProvider):
    def __init__(self, settings: Settings) -> None:
        if settings.provider is not ProviderName.GROQ:
            raise ValueError("GroqProvider requires MODEL_PROVIDER=groq")
        self._settings = settings
        self._agent = StrandsTicketAgent(settings)

    @property
    def name(self) -> str:
        return ProviderName.GROQ.value
